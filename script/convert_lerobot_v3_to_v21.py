...
"""Convert a local LeRobot dataset from format v3.0 to v2.1.

Usage examples
--------------

- The source dataset is never modified. The v2.1 result is written to a new
  directory selected with ``--output-root``.
...
try:
    ...
    if exc.name is not None and (exc.name == "lerobot" or exc.name.startswith("lerobot.")):
        raise ModuleNotFoundError(
            "Install a LeRobot release containing the v3.0 dataset utilities, "
            "then rerun this command. See README.md for the supported setup."
        ) from None
    raise
...
def convert_dataset(
    ...
    new_root = Path(output_root).expanduser().resolve()
    if not root.is_dir():
    ...
    if new_root.exists():
        raise FileExistsError(
            f"Output directory already exists: {new_root}. "
            "Choose a new path so conversion cannot overwrite existing data."
        )
    ...
    logging.info("Converted v2.1 dataset written to %s", new_root)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert a local LeRobot v3.0 dataset to a new v2.1 directory."
    )
    parser.add_argument(
        "--input-root",
        type=Path,
        required=True,
        help="Existing local LeRobot v3.0 dataset root.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        required=True,
        help="New directory for the converted LeRobot v2.1 dataset.",
    )
