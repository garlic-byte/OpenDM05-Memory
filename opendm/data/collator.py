...
class TrainingCollator:
    ...
    def __call__(self, instances: Sequence[dict]) -> dict[str, torch.Tensor]:
        ...
        history_pixels = [
            instance["history_pixel_values"]
            for instance in instances
            if instance.get("history_pixel_values") is not None
        ]
        if history_pixels:
            batch["history_pixel_values"] = torch.cat(history_pixels, dim=0)

        if any(instance.get("history_mask") is not None for instance in instances):
            padded_history_masks = []
            for instance in instances:
                history_mask = instance.get("history_mask")
                if history_mask is None:
                    history_mask = torch.zeros_like(
                        instance["input_ids"], dtype=torch.bool
                    )
                history_mask = history_mask[:, :max_len]
                pad_len = max_len - history_mask.shape[1]
                if pad_len > 0:
                ...
                padded_history_masks.append(history_mask)
            batch["history_mask"] = torch.cat(padded_history_masks, dim=0)
